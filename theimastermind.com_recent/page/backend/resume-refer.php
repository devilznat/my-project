...
//*** Update Last Stay in Login System
  ...
  if ($status==0) {
  		$number="SELECT * FROM resumes WHERE (status_resume = 1 OR status_resume = 2) ORDER BY resume_id ";

  	# code...
  } 

  else if ($status==1) {
  		$number="SELECT * FROM resumes WHERE (status_resume = 1 OR status_resume = 2) AND first_name like '%$gettext%' OR surname like '%$gettext%'  ";
  	# code...
  }
  else if ($status==2) {
  		$number="SELECT * FROM resumes WHERE (status_resume = 1 OR status_resume = 2) AND created_date BETWEEN '$getdatestart' AND '$getdateend' AND (first_name like '%$gettext%' OR surname like '%$gettext%')  ";
  	# code...
  }
...
$start=($page-1)*$rows;
  ...
  if ($status==0) {
  		...
  		$sql = "SELECT * FROM resumes WHERE (status_resume = 1 OR status_resume = 2) ORDER BY resume_id DESC limit $start,10";
  	# code...
  } 

  else if($status==1) {

  		$sql = "SELECT * FROM resumes WHERE (status_resume = 1 OR status_resume = 2) AND first_name like '%$gettext%' OR surname like '%$gettext%'  ORDER BY resume_id DESC limit $start,10";
  }
  else if($status==2) {

  	$sql = "SELECT * FROM resumes WHERE (status_resume = 1 OR status_resume = 2) AND created_date BETWEEN '$getdatestart' AND '$getdateend' AND (first_name like '%$gettext%' OR surname like '%$gettext%') limit $start,10";
  }
...
<body class="header-fixed header-fixed-space">
	<div class="wrapper">
		...
		<div class="breadcrumbs">
			<div class="container">
				<h1 class="pull-left">Resumes-Refer-Jobs Page</h1>
		...
		?>
									<tr>
										...
										?>
											...
											<b><u>Job Refer</u> : <br>
											 <font color="#446cb3"><? echo $loadjob1[title]; ?><br></b></font>
											 <font color="red"><b><u>Job ID</u> : <? echo $loadjob1[job_id]; ?></b></font>
											<br> 
										<?
										}
										?>
										...
										?>
											<button class="btn btn-success btn-xs" onclick="window.location.href='resume-detail?idresume=<? echo $rs["resume_id"]; ?>'">
    ...
    <li <? if($page==1) echo 'class="page-item disabled"';?>>
    	...
    	<?
    		...
    		if ($status == 0 ) {
    			...
    			$pagenumback = "resume-refer?page=".$numback;
  				
    		} else {
    			
    			$pagenumback = "resume-refer?search=1&textsearch=".$gettext."&page=".$numback;
    			# code...
    		}
    ...
    <li <? 
    	if($page==$i) echo 'class="page-item active"';?> >
    		...
    		if ($status == 0 ) {
    			...
    			$pagenumnext = "resume-refer?page=".$i;
  				
    		} else {
    		
    			$pagenumnext = "resume-refer?search=1&textsearch=".$gettext."&page=".$i;
    			# code...
    		}
...
<script type="text/javascript">
			...
			function Searchresume(event, form){
				...
				if (datestart == "" ) {
					...
					url = "resume-refer?search="+search+"&textsearch="+textsearch;
					...
					url = "resume-refer?search="+search+"&textsearch="+textsearch+"&textdatestart="+datestart+"&textdateend="+dataend;
					 window.location.assign(url);
